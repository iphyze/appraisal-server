...
try {
    ...
    if ($cycleId <= 0) {
        jsonResponse('Success', 'Select a cycle to load staff assignments.', [], $emptyMeta);
    }
    $allSupervisors = $supervisorId <= 0;

    $companyScope = resolveCompanyScope($userData);
    $cycleCompanySql = $companyScope !== null
        ? " AND company_id = " . (int) $companyScope
        : "";
    ...
    // A scoped admin may manage all peer appraisers but only their scoped regular staff.
    $adminScope = $loggedRoleKey === 'admin' ? trim((string) ($userData['staff_scope'] ?? 'All')) : 'All';
    if (in_array($adminScope, ['Local', 'Expatriate'], true)) {
        $safeAdminScope = esc($conn, $adminScope);
        $where[] = "(" . roleKeySql('sr') . " <> 'staff' OR s.staff_type = '{$safeAdminScope}')";
    }
    ...
    $assignmentJoins = "
        LEFT JOIN supervisor_assignments cur
           ...
           AND ({$supervisorId} <= 0 OR cur.supervisor_id = {$supervisorId})
    ...
    if (in_array($adminScope, ['Local', 'Expatriate'], true)) {
        $safeAdminScope = esc($conn, $adminScope);
        $statsWhere[] = "(" . roleKeySql('sr') . " <> 'staff' OR s.staff_type = '{$safeAdminScope}')";
    }
    ...
    $supervisorMeta = $allSupervisors ? ['id' => 0, 'first_name' => 'All', 'last_name' => 'Supervisors', 'department' => 'All departments', 'job_title' => 'Overview'] : array_merge($supervisor, [
